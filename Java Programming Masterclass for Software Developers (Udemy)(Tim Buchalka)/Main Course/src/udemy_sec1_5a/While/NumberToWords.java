...
As seen in a previous example, 100 reversed becomes 1, but the method numberToWords
should print "One Zero Zero". To get the number of zeroes, check the difference between
...
NOTE: The methods numberToWords, getDigitCount, reverse
...
public class NumberToWords
{
    public static void numberToWords(int number)
    {
        ...
        {
            int toPrint = 0;
            int toReversed = reverse(number);
            while (toReversed != 0)
            {
                toPrint = toReversed % 10;
                toReversed /= 10;
                switch (toPrint)
                {
                    case 0:
            ...
            toReversed = reverse(number);
            for(int i = getDigitCount(toReversed); i < getDigitCount(number); i++)
            {
                System.out.println("Zero");
            }
        }
    ...
    public static int reverse(int number)
    {
        ...
        if (number < 0) plusSign = false;
        int corrected = 0;
        while (Math.abs(number) != 0)
        {
            corrected = corrected * 10 + Math.abs(number) % 10;
            number = Math.abs(number) / 10;
        }
        if (plusSign == false) corrected *= -1;
        return corrected;
    }
