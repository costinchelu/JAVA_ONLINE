...
public class CnpValidator {
    ...
    public static boolean validareCnp(String cnp) {
        ...
        List<Integer> cnpSplit = Arrays.stream(cnp.split("")).map(Integer::parseInt).collect(Collectors.toList());
        List<Integer> valueSplit = Arrays.stream(STD_VALUE.split("")).map(Integer::parseInt).collect(Collectors.toList());
        int sum = 0;
        int i = 0;
